...
class Listener:
    ...
    def __init__(self, wake_word: str = "hey matrix", config: Optional[WakeWordConfig] = None):
        ...
        self.is_speaking = False  # NEW: track when speech is playing
    ...
    def speak(self, text: str):
        """Speak text asynchronously while still listening"""
        def _speak():
            try:
                with self.lock:
                    self.is_speaking = True
                    log_debug("Speech started, listener temporarily passive")

                self.speech.speak(text)  # blocking call to TTS
                ...
                with self.lock:
                    ...
                    log_debug("Speech finished, listener active again")
    ...
    def _handle_detection(self, text: str, confidence: float) -> bool:
        ...
        # Ignore detections while speaking
        with self.lock:
            if self.is_speaking:
                log_debug("Ignored wake word during speech playback")
                return False
        ...
        log_info(f"Wake word detected ({confidence:.2%}): '{text}'")
    ...
    def wait_for_wake_word(self, timeout: Optional[int] = None) -> Optional[str]:
        ...
        while self.is_listening:
            try:
                ...
                with self.lock:
                    if self.is_speaking:
                        # skip listening during speech output
                        time.sleep(0.5)
                        continue
